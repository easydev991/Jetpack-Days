...
11. ❌ **Баг #11**: DetailScreen не возвращает на главный экран после сохранения
12. ❌ **Баг #12**: Свайп слева направо открывает несколько экранов редактирования
...
Список записей на главном экране перекрывается верхним тулбаром - надо поправить, чтобы список "уважал" тулбар и был ниже по оси Y (стандартными способами).

### Причина

Проблема в верстке Scaffold:
- TopAppBar внутри Scaffold перекрывает контент
- Отсутствует правильный отступ для списка под AppBar
...
## Баг #6: Неправильная верстка ListItemView
...
Проблема в верстке ListItemView:
- Отсутствует ограничение ширины для текста справа
- Тексты могут быть слишком длинными
- Нет ограничений на количество строк

### Шаги исправления

#### Шаг 6.1: Анализ текущей верстки
...
#### Шаг 6.2: Исправление верстки

**Задача:** Реализовать правильную верстку

**Действия:**
1. Ограничить ширину для текста с количеством дней (30% от ширины экрана)
2. Добавить ограничение на 2 строки для названия и описания на главном экране
3. Использовать `TextOverflow.Ellipsis` для обрезки длинного текста
4. Убедиться, что описание занимает не более 30% высоты элемента списка
...
Проблема в RootScreen:
- NavigationBar реализована, но может быть скрыта или неправильно настроена
- Отсутствует проверка видимости или логика переключения экранов
...
## Баг #8: Контент заезжает под камеру

### Описание

При повороте экрана контент заезжает под переднюю камеру - можно настроить "безопасную" зону стандартными инструментами?

### Причина

Отсутствует padding для контента под StatusBar / Notch

### Шаги исправления

#### Шаг 8.1: Анализ проблемы

**Задача:** Понять, почему контент заезжает

**Действия:**
1. Проверить использование `WindowInsets`
2. Изучить верстку Scaffold во всех экранах
3. Проверить наличие отступов

#### Шаг 8.2: Добавление padding под StatusBar

**Задача:** Обеспечить правильные отступы

**Действия:**
1. Добавить `WindowInsets.systemBars` во все экраны
2. Использовать `Scaffold(contentWindowInsets = ...)` для безопасного отображения контента
3. Добавить отступ под переднюю камеру вверху экрана

**Критерий готовности:**
- ✅ Контент не заезжает под камеру
- ✅ Используется безопасная зона для отображения
...
## Баг #11: DetailScreen не возвращает на главный экран после сохранения

### Описание

В DetailScreen после сохранения изменений запись не возвращается на главный экран.

### Причина

После сохранения изменений в DetailScreen должен происходить возврат на MainScreen, но этого не происходит.

### Шаги исправления

#### Шаг 11.1: Анализ навигации из DetailScreen

**Задача:** Понять, почему нет возврата

**Действия:**
1. Изучить DetailScreen.kt
2. Проверить обработчик кнопки сохранения
3. Проверить навигацию в RootScreenComponents.kt

#### Шаг 11.2: Добавление возврата на главный экран

**Задача:** Обеспечить возврат на главный экран

**Действия:**
1. Добавить обработчик для кнопки "Назад" после сохранения
2. Убедиться, что после сохранения происходит навигация на MainScreen
3. Проверить, что при сохранении без изменений также происходит возврат

**Критерий готовности:**
- ✅ После сохранения в DetailScreen происходит возврат на MainScreen
- ✅ Измененная запись отображается в списке
- ✅ Пользователь возвращается на главный экран

---

## Баг #12: Свайп слева направо открывает несколько экранов редактирования
...
#### Шаг 12.1: Анализ текущей реализации свайпа для редактирования
...
#### Шаг 12.2: Исправление навигации при свайпе
...
#### Шаг 12.3: Исправление состояния свайпа и выделения
...
#### Шаг 12.4: Исправление возврата на главный экран
