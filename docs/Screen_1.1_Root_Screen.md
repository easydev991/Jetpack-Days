...
- `ui/screens/root/RootScreen.kt` — RootScreen composable
- `ui/screens/common/RootScreenComponents.kt` — компоненты навигации
...
- **Локализация**: Используются строковые ресурсы (`events`, `more`, `app_theme_and_icon`, `app_data`) через `titleResId` в `res/values/strings.xml` и `res/values-ru/strings.xml`
...
- **Открытие из уведомления**: RootScreen принимает `pendingOpenDetailItemId` и открывает `ItemDetail` при переходе по reminder-уведомлению
...
- 2026-09-16: Актуализация — добавлено открытие ItemDetail из reminder-уведомления (`pendingOpenDetailItemId`)
