# Экран 5.1: More Screen (Ещё)

## Обзор

More Screen является экраном с дополнительными функциями и настройками приложения. Экран предоставляет доступ к настройкам темы и иконки, управлению данными, а также к внешним ссылкам и функциям обратной связи.

## Назначение

Экран с дополнительными функциями и настройками приложения, включая настройки темы и иконки, управление данными, отправку отзыва и ссылки на внешние ресурсы.

## Компоненты

1. **Заголовок экрана** — "Ещё" (используется из NavigationBar, не дублируется)

2. **Список кнопок настроек и действий**:

   - **Кнопка "Тема и иконка"** — Заглушка, выводит в консоль "TODO: открыть экран ThemeIcon Screen"
   - **Кнопка "Данные приложения"** — Заглушка, выводит в консоль "TODO: открыть экран AppData Screen"
   - **Кнопка "Отправить отзыв"** — Открывает почтовый клиент/форму обратной связи
   - **Кнопка "Оценить приложение"** — Ссылка на страницу в Google Play для оценки
   - **Кнопка "Поделиться приложением"** — ShareSheet с ссылкой на приложение в Google Play
   - **Кнопка "Страница на GitHub"** — Ссылка на репозиторий приложения

3. **Текст версии приложения** (внизу экрана)

   - Формат: "Версия приложения: X.Y.Z" (локализован через stringResource)

## Навигация

- "Тема и иконка" → Заглушка (выводит "TODO: открыть экран ThemeIcon Screen" в консоль)
- "Данные приложения" → Заглушка (выводит "TODO: открыть экран AppData Screen" в консоль)

**Примечание:** Реальная навигация к экранам 5.2 и 5.3 не реализована в рамках этого этапа. Эти кнопки являются заглушками для будущего функционала.

## Зависимости от других этапов

- ✅ **Экран 1.1: Root Screen** — уже реализован, навигация к More Screen настроена
- ⚠️ **Экран 5.2: ThemeIcon Screen** — не требуется для этого этапа (кнопка-заглушка)
- ⚠️ **Экран 5.3: AppData Screen** — не требуется для этого этапа (кнопка-заглушка)

---

## Подробный план реализации по TDD

### Шаг 1: Структура и навигация

**Статус:** ⚠️ Частично выполнено

#### 1.1. Создание файла экрана

**Задачи:**

1. ~~Создать отдельный файл `MoreScreen.kt` в `ui/screen/`~~ - Требуется, в данный момент компонент находится в `RootScreenComponents.kt`
2. ~~Вынести `moreScreenContent()` из RootScreen.kt в отдельный компонент~~ - Уже реализовано в `RootScreenComponents.kt` (строки 187-209), но только как заглушка

**Текущее состояние:**

- ✅ Заглушка `moreScreenContent()` существует в `RootScreenComponents.kt` (строки 187-209)
- ✅ Маршрут Screen.More уже существует в navigation/Screen.kt (строки 23-27)
- ❌ Нет отдельного файла MoreScreen.kt
- ⚠️ Заглушка отображает только текст "Экран настроек не реализован"

**Критерии готовности:**

- ✅ Файл MoreScreen.kt создан
- ✅ Компонент вынесен из RootScreenComponents.kt

---

#### 1.2. Настройка навигации

**Задачи:**

1. Реализовать кнопку-заглушку для "Тема и иконка" (выводит в консоль "TODO: открыть экран ThemeIcon Screen") - Требуется
2. Реализовать кнопку-заглушку для "Данные приложения" (выводит в консоль "TODO: открыть экран AppData Screen") - Требуется

**Текущее состояние:**

- ✅ Навигационная панель с двумя вкладками (Events, More) уже настроена
- ✅ Базовая навигация к экрану More уже работает
- ❌ Нет кнопок-заглушек для ThemeIcon и AppData экранов

**Критерии готовности:**

- ✅ Кнопки-заглушки для ThemeIcon и AppData реализованы
- ✅ При нажатии выводятся соответствующие сообщения в консоль
- ✅ Нет крашей при нажатии на кнопки-заглушки

---

### Шаг 2: Реализация UI компонентов

**Статус:** ❌ Не начат

#### 2.1. Создание списка кнопок

**Задачи:**

1. Заменить заглушку в `moreScreenContent()` на полноценную реализацию UI
2. Создать список кнопок с текстовыми метками:

   - "Тема и иконка"
   - "Данные приложения"
   - "Отправить отзыв"
   - "Оценить приложение"
   - "Поделиться приложением"
   - "Страница на GitHub"

3. Реализовать верстку компонентов экрана:

   - Использовать LazyColumn для основного макета с возможностью вертикальной прокрутки
   - Список должен быть прокручиваемым, особенно в горизонтальной ориентации устройства
   - Добавить список кнопок с текстовыми метками
   - Обеспечить визуальное разделение между группами элементов (spacing: 16dp)
   - Добавить отображение версии приложения внизу экрана (foregroundStyle: secondary)

**Текущее состояние:**

- ❌ Только заглушка с текстом "Экран настроек не реализован"
- ❌ Нет списка кнопок с текстом
- ❌ Нет прокручиваемого списка

**Критерии готовности:**

- ✅ Заглушка заменена на полноценный UI
- ✅ Список кнопок создан с текстовыми метками
- ✅ Верстка выполнена с LazyColumn
- ✅ Список прокручивается вертикально (особенно в горизонтальной ориентации)
- ✅ Верстка идентична iOS для единообразия UX

---

### Шаг 3: Реализация функциональности

**Статус:** ❌ Не начат

#### 3.0. Создание констант для внешних ссылок

**Задачи:**

1. Создать файл `AppConstants.kt` в пакете `com/dayscounter/util/`
2. Добавить константы для внешних ссылок:

   ```kotlin
   object AppConstants {
       const val APP_STORE_URL = "https://play.google.com/store/apps/details?id=com.dayscounter"
       const val GITHUB_REPOSITORY_URL = "https://github.com/easydev991/Jetpack-Days"
   }
   ```

3. Обеспечить доступность констант для использования в UI компонентах

**Текущее состояние:**

- ❌ Файл AppConstants.kt не создан
- ❌ Константы для ссылок не определены

**Критерии готовности:**

- ✅ Файл AppConstants.kt создан в пакете util/
- ✅ Все необходимые константы определены (App Store, GitHub)
- ✅ Константы используются в коде экрана More Screen

**Примечание:** Константы должны быть в удобном месте для легкой настройки ссылок при необходимости изменения.

---

#### 3.1. Оценка приложения

**Задачи:**

1. Реализовать оценку приложения (открытие страницы в Google Play)
2. Использовать константу `AppConstants.APP_STORE_URL`
3. Обработать случаи, когда Google Play не доступен

**Текущее состояние:**

- ❌ Не реализовано

**Критерии готовности:**

- ✅ Оценка приложения работает через Intent.ACTION_VIEW
- ✅ Обработка ошибок реализована
- ✅ Используется константа `AppConstants.APP_STORE_URL`

---

#### 3.2. Шеринг приложения

**Задачи:**

1. Реализовать шеринг приложения (ShareSheet с ссылкой на Google Play)
2. Использовать константу `AppConstants.APP_STORE_URL`
3. Обработать случаи ошибок

**Текущее состояние:**

- ❌ Не реализовано

**Критерии готовности:**

- ✅ Шеринг работает через Intent
- ✅ Обработка ошибок реализована
- ✅ Используется константа `AppConstants.APP_STORE_URL`

---

#### 3.3. Открытие GitHub репозитория

**Задачи:**

1. Реализовать открытие GitHub репозитория (открытие ссылки в браузере)
2. Использовать константу `AppConstants.GITHUB_REPOSITORY_URL`
3. Обработать случаи ошибок

**Текущее состояние:**

- ❌ Не реализовано

**Критерии готовности:**

- ✅ Открытие GitHub работает через Intent
- ✅ Обработка ошибок реализована
- ✅ Используется константа `AppConstants.GITHUB_REPOSITORY_URL`

---

#### 3.4. Отправка отзыва

**Задачи:**

1. Реализовать отправку отзыва (открытие почтового клиента/формы обратной связи)
2. Использовать Intent.ACTION_SENDTO для отправки email
3. Обработать случаи, когда почтовый клиент не установлен

**Текущее состояние:**

- ❌ Не реализовано

**Критерии готовности:**

- ✅ Отправка отзыва работает через Intent
- ✅ Обработка ошибок реализована (клиент не установлен)

---

#### 3.5. Отображение версии приложения

**Задачи:**

1. Отобразить текущую версию приложения (получение из BuildConfig.VERSION_NAME)
2. Форматировать версию через stringResource(R.string.app_version, BuildConfig.VERSION_NAME)

**Текущее состояние:**

- ❌ Не реализовано

**Критерии готовности:**

- ✅ Версия отображается внизу экрана
- ✅ Форматирование работает корректно с локализованным ресурсом

---

### Шаг 4: Локализация

**Статус:** ✅ Выполнен

**Задачи:**

1. ~~Использовать готовые строковые ресурсы из `Localization_Plan.md`:~~

   - ~~Заголовок экрана: `more`~~ - ✅ Уже используется в Screen.kt
   - ~~Названия кнопок: `app_theme_and_icon`, `app_data`, `send_feedback`, `rate_the_app`, `share_the_app`, `github_page`~~ - ✅ Уже существуют в strings.xml
   - ~~Формат версии приложения: `app_version` (с параметром)~~ - ✅ Уже существует в strings.xml

2. ~~Поддержка русского и английского языков~~ - ✅ Уже реализована в `Localization_Plan.md`

**Текущее состояние:**

- ✅ Все строковые ресурсы существуют в `values/strings.xml`, `values-ru/strings.xml`, `values-en/strings.xml`
- ✅ Локализация уже реализована и интегрирована в проект
- ✅ Ключи ресурсов: `app_version`, `send_feedback`, `rate_the_app`, `share_the_app`, `github_page`, `app_data`, `app_theme_and_icon`

**Критерии готовности:**

- ✅ Используются готовые строковые ресурсы
- ✅ Локализация работает корректно

**Примечание:** Все строки определены в `Localization_Plan.md` (Шаги 1-6 выполнены). Необходимо только интегрировать их в MoreScreen.kt.

---

### Шаг 5: Финальное тестирование

**Статус:** ❌ Не начат

**Задачи:**

1. Проверить кнопку-заглушку для "Тема и иконка" (UI тесты) - проверка вывода в консоль
2. Проверить кнопку-заглушку для "Данные приложения" (UI тесты) - проверка вывода в консоль
3. Проверить открытие почтового клиента для отзыва (UI тесты)
4. Проверить открытие Google Play для оценки (UI тесты)
5. Проверить функцию шеринга (UI тесты)
6. Проверить открытие GitHub репозитория (UI тесты)
7. Проверить отображение версии приложения (UI тесты)
8. Проверить прокрутку списка в горизонтальной ориентации (UI тесты)
9. Написать unit-тесты для логики открытия Intents

**Текущее состояние:**

- ❌ Нет UI тестов
- ❌ Нет unit тестов

**Критерии готовности:**

- ✅ Все тесты проходят
- ✅ Все функции работают корректно

---

## Реализация (детали)

### UI компоненты

- Использовать LazyColumn для прокрутки содержимого с возможностью вертикальной прокрутки
- Список должен быть прокручиваемым, особенно в горизонтальной ориентации устройства
- Добавить список кнопок с текстовыми метками
- Обеспечить визуальное разделение между группами элементов (Spacer, Divider)
- Добавить отображение версии приложения внизу экрана (Text с LocalContentAlpha.medium)
- Использовать современные компоненты Material Design 3 (MaterialTheme, Card, ListItem)
- Применить тему приложения с поддержкой темного режима (MaterialTheme.colorScheme)
- Верстка идентична iOS для единообразия UX

### Кнопки-заглушки для настроек

В рамках этого этапа экраны "Тема и иконка" и "Данные приложения" не реализуются. Вместо этого создаются кнопки-заглушки:

**"Тема и иконка":**

- Кнопка отображается как обычный элемент списка
- При нажатии выводится сообщение в консоль: "TODO: открыть экран ThemeIcon Screen"
- Никаких крашей или ошибок не возникает
- Реальная навигация не реализована

**"Данные приложения":**

- Кнопка отображается как обычный элемент списка
- При нажатии выводится сообщение в консоль: "TODO: открыть экран AppData Screen"
- Никаких крашей или ошибок не возникает
- Реальная навигация не реализована

**Пример реализации кнопки-заглушки:**

```kotlin
Button(
    onClick = {
        println("TODO: открыть экран ThemeIcon Screen")
    }
) {
    Text(stringResource(R.string.app_theme_and_icon))
}
```

**Примечание:** Реализация экранов ThemeIcon Screen и AppData Screen будет выполнена в отдельных документах (Screen_5.2 и Screen_5.3). В этом этапе эти кнопки служат только заглушками для будущего функционала.

### Архитектура

- MoreScreen не требует ViewModel (простой экран без состояния)
- Использовать composable-функции с параметрами для навигации
- NavHostController передается через параметры или из LocalNavController
- Обработка ошибок происходит в UI-слое через try-catch для Intent действий
- ~~Clean Architecture (Data → Domain → Presentation), если требуется для будущих функций~~ → Не требуется для этого экрана

---

## Тестирование

### Unit-тесты

- ~~Тесты для логики получения версии приложения~~ → Не требуется (используется BuildConfig.VERSION_NAME)
- Тесты для обработки ошибок внешних ссылок (Intents)
- Тесты для форматирования версии приложения с stringResource

### UI-тесты

- Проверить наличие всех кнопок на экране
- Проверить кнопку-заглушку "Тема и иконка" (выводит в консоль "TODO: открыть экран ThemeIcon Screen")
- Проверить кнопку-заглушку "Данные приложения" (выводит в консоль "TODO: открыть экран AppData Screen")
- Проверить открытие почтового клиента для отзыва
- Проверить открытие Google Play для оценки
- Проверить функцию шеринга
- Проверить открытие GitHub репозитория
- Проверить отображение версии приложения
- Проверить прокрутку списка в горизонтальной ориентации

---

## Критерии завершения этапа

Этап считается завершенным, когда:

- ✅ Файл MoreScreen.kt создан и функционален
- ✅ Все UI компоненты реализованы
- ✅ Все функции (отправка отзыва, оценка, шеринг, GitHub) работают
- ✅ Кнопки-заглушки для "Тема и иконка" и "Данные приложения" реализованы с выводом в консоль
- ✅ Все строковые ресурсы локализованы
- ✅ Все unit-тесты написаны и проходят
- ✅ Все UI-тесты написаны и проходят
- ✅ Код соответствует правилам проекта
- ✅ Линтеры не выдают ошибок
- ✅ Все внешние ссылки работают корректно

---

## Блокируемые этапы

После завершения этого этапа можно приступать к:

- Экран 5.2: ThemeIcon Screen (реализуется отдельно)
- Экран 5.3: AppData Screen (реализуется отдельно)

**Примечание:** Экраны 5.2 и 5.3 реализуются отдельно в своих документах. В рамках этого этапа кнопки "Тема и иконка" и "Данные приложения" являются только заглушками с выводом в консоль.

---

## Текущее состояние реализации

**На основе анализа кода от января 2026 года:**

### Выполнено

- ✅ Экран Root Screen реализован и работоспособен
- ✅ Навигационная панель с двумя вкладками (Events, More) настроена
- ✅ Маршрут Screen.More определен в navigation/Screen.kt
- ✅ Заглушка moreScreenContent() реализована в RootScreenComponents.kt
- ✅ Все строковые ресурсы для локализации существуют (values/strings.xml, values-ru/strings.xml, values-en/strings.xml)
- ✅ Локализация интегрирована в проект (Localization_Plan.md шаги 1-6 выполнены)

### Не выполнено

- ❌ Отсутствует отдельный файл MoreScreen.kt (компонент в RootScreenComponents.kt)
- ❌ moreScreenContent() содержит только заглушку "Экран настроек не реализован"
- ❌ Нет UI компонентов (кнопки с текстом, прокручиваемый список)
- ❌ Не создан файл AppConstants.kt с константами для внешних ссылок
- ❌ Не реализована функциональность (отправка отзыва, оценка, шеринг, GitHub)
- ❌ Не отображается версия приложения
- ❌ Нет unit тестов
- ❌ Нет UI тестов

### Приоритет задач

1. **Высокий приоритет:** Создание файла MoreScreen.kt и базовой верстки UI
2. **Высокий приоритет:** Реализация кнопок-заглушек для "Тема и иконка" и "Данные приложения"
3. **Средний приоритет:** Реализация функциональности кнопок (отправка, шеринг и т.д.)
4. **Низкий приоритет:** Написание тестов

---

## Примечания

1. **Внешние ссылки**: Все внешние ссылки должны обрабатывать случаи, когда соответствующие приложения не установлены или недоступны. Использовать try-catch для Intent действий.

2. **Константы для ссылок**: Все внешние ссылки (App Store, GitHub) должны храниться в константах в файле `AppConstants.kt` (пакет `com/dayscounter/util/`). Это позволяет легко изменять ссылки в одном месте без поиска по всему коду. Константа `APP_STORE_URL` используется для оценки и шеринга приложения.

3. **Версия приложения**: Версия должна получаться из BuildConfig.VERSION_NAME для автоматического обновления. Форматирование через stringResource(R.string.app_version, BuildConfig.VERSION_NAME).

4. **Тестирование**: Все компоненты должны быть покрыты тестами перед использованием в других этапах.

5. **Зависимости**: Реализация зависит от завершения Экрана 1.1 (Root Screen) - ✅ выполнено.

6. **Архитектура**: MoreScreen не требует ViewModel из-за отсутствия сложного состояния. Использовать Stateless подход с передачей параметров.

7. **Локализация**: Все строки уже существуют в проекте, необходимо только использовать их в коде (stringResource).

8. **Прокрутка списка**: Список кнопок должен быть реализован в LazyColumn с возможностью вертикальной прокрутки. Это особенно важно для горизонтальной ориентации устройства, когда список может не умещаться целиком на экране.

9. **Кнопки-заглушки для экранов настроек**: В рамках этого этапа не реализуется реальная навигация к экранам "Тема и иконка" (Screen 5.2) и "Данные приложения" (Screen 5.3). Вместо этого создаются кнопки-заглушки, которые:
   - Отображаются как обычные элементы списка
   - При нажатии выводят в консоль сообщение "TODO: открыть экран {ThemeIcon|AppData} Screen"
   - Не вызывают крашей или ошибок
   - Реализованы без реальной навигации

   Реальные экраны будут реализованы в отдельных документах (Screen_5.2_Theme_Icon_Screen.md и Screen_5.3_App_Data_Screen.md).
