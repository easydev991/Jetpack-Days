...
**ВАЖНО:** Подробные правила тестирования см. в `.agents/rules/tdd.mdc`
...
**Примечание:** Интеграционные тесты ViewModels не создаются (см. `.agents/rules/tdd.mdc`). ViewModel тестируется через unit-тесты с MockK.
