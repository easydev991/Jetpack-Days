...
- `listItemView()` composable с параметрами: `item`, `formattedDaysText`, `onClick`
...
**Реализовано в:** `topAppBar()` в `MainScreen.kt`
...
**См. раздел "Приоритет 4" ниже для описания задач UI-тестов.**
...
**Задачи:**
- Запустить `./gradlew ktlintCheck`
- Запустить `./gradlew detekt`
