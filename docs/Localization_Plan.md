...
Данный документ описывает план реализации локализации для Android-приложения на основе строк из iOS-приложения. Все строки уже определены в iOS-проекте (`Localizable.xcstrings`) и должны быть перенесены в Android-проект с сохранением идентичности переводов.
