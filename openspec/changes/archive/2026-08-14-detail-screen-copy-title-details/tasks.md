...
- [x] 4.3 Модифицировать `ReadSectionView`: добавить `onCopy: (() -> Unit)? = null`, обернуть `Text` в `Box` с `Modifier.onSizeChanged { textHeightPx = it.height }`, навесить `Modifier.pointerInput` с `detectTapGestures(onLongPress = { menuOffset = with(density) { DpOffset(touchOffset.x.toDp(), (touchOffset.y - textHeightPx).toDp()) }; menuVisible = true })` только при `onCopy != null`, рендерить `DropdownMenu` с `offset = menuOffset` и одним `DropdownMenuItem` ("Скопировать" + ContentCopy иконка) только при `onCopy != null`. Один `Text` рендерится всегда — `pointerInput` и `DropdownMenu` вешаются условно (нет дублирующей `else`-ветки с `Text`). Позиционирование: Material3 1.4 `DropdownMenu` использует `topToAnchorBottom` (`menu.top = anchor.bottom + offset.y`); вычитание `textHeightPx` из `offset.y` компенсирует сдвиг и даёт попадание top-left меню в точку касания (`menu.top = Box.top + touchOffset.y = touch.Y`). Добавить KDoc на новые публичные параметры `ReadSectionView` и `DetailContentByState` согласно AGENTS.md «KDoc for public APIs».
