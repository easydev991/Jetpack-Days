...
Парсинг через `kotlinx.serialization` (`@Serializable data class GitHubRelease(@SerialName("tag_name") val tagName: String? = null, @SerialName("html_url") val htmlUrl: String? = null, val body: String? = null)` в `domain/usecase/http/` — рядом с `HttpRequestExecutor`, по прецеденту `BackupItem` в `domain/usecase/`, см. tasks.md:1.3), `Json { ignoreUnknownKeys = true }` для устойчивости к новым полям API. При отсутствии `tag_name` или `html_url` в ответе (теоретически невозможно по контракту API, но defensive — на случай частично-повреждённого JSON или будущих изменений API) — `Result.failure(AppUpdateException("Поле 'tag_name'/'html_url' отсутствует в ответе GitHub"))`. Nullable-поля с явным default `= null` делают ключ опциональным: в kotlinx-serialization nullable-поле без default — обязательный ключ (отсутствие ключа → `MissingFieldException`, ловится общим `runCatching` без указания поля), а с `= null` парсер не падает и null попадает в защитную проверку в use case, которая даёт диагностику с именем поля.
