...
- [x] 3.2 Создать `internal @Composable fun rememberCopyToClipboardHandler(clipboardHelper: ClipboardHelper = SystemClipboardHelper()): (label: String, message: String, text: String) -> Unit`. Внутри lambda: `clipboardHelper.copy(context, label, text)`; при `Result.success(Unit)` И `Build.VERSION.SDK_INT < Build.VERSION_CODES.TIRAMISU` — `Toast.makeText(context, message, Toast.LENGTH_SHORT).show()`. На API ≥33 (Android 13+) системный Toast НЕ показывается — ОС сама показывает системный overlay. Без `coroutineScope` (handler синхронный, см. `AppDataScreen.kt:109`). `message` — уже готовый `String`, резолвленный вызывающим через `stringResource(...)` в composable-скоупе; handler не вызывает `context.getString()` сам (иначе lint `ConfigurationLocale` и stale-значение при смене локали).
