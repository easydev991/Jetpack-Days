...
Пользователь на экране `DetailScreen` может через долгое нажатие на текст секции `Title` или `Details` скопировать значение в системный буфер обмена; на устройствах с Android 12 и старше (API <33) дополнительно показывается системный `Toast` с подтверждением копирования.
