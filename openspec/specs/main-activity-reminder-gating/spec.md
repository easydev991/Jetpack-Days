# Main Activity Reminder Gating

## Purpose

Define how `MainActivity.onCreate` distinguishes a true cold-start (process-restart, launcher, push-tap on killed app) from Activity recreation (rotation, theme change, configuration change, return-from-background process restore) so that `handleReminderIntent` is invoked exactly once per push delivery, and a previously-opened `DetailScreen` is not re-pushed onto the navigation stack after a configuration change.
